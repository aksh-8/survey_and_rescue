...
class sr_determine_rois():
	...
	def detect_rois(self):
		...
		img_copy1 = cv2.cvtColor(img_copy,cv2.COLOR_GRAY2BGR)
		dst = cv2.fastNlMeansDenoisingColored(img_copy1, None, 10, 10, 7, 15) 
		cv2.imshow("Detected ROIs", dst)
		cv2.waitKey(0)
